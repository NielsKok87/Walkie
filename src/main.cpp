 ...
 *   DIN  -> GPIO 7  (I2S_SD_OUT - Speaker data)
...
#define I2S_WS          5   // Word Select / LRCLK (Shared)

// I2S Data Pins (separate for input and output)
#define I2S_SD_IN       6   // Serial Data from microphone (INMP441)
#define I2S_SD_OUT      7   // Serial Data to speaker (MAX98357A)
...
void setupI2S_Microphone() {
  ...
  i2s_pin_config_t pin_config_mic = {
    .bck_io_num = I2S_BCLK,       // Shared bit clock
    .ws_io_num = I2S_WS,          // Shared word select
    .data_out_num = I2S_PIN_NO_CHANGE,
    .data_in_num = I2S_SD_IN      // Separate mic data input
  };
...
void setupI2S_Speaker() {
  ...
  i2s_pin_config_t pin_config_spk = {
    .bck_io_num = I2S_BCLK,       // Shared bit clock
    .ws_io_num = I2S_WS,          // Shared word select
    .data_out_num = I2S_SD_OUT,   // Separate speaker data output
...
void audioRxTask(void *parameter) {
  ...
  while (true) {
    ...
    if (xQueueReceive(audioRxQueue, &rxBuffer, pdMS_TO_TICKS(100)) == pdTRUE) {
      ...
      i2s_write(I2S_PORT_SPK, rxBuffer.data, rxBuffer.size * 2, &bytes_written, portMAX_DELAY);
    }
